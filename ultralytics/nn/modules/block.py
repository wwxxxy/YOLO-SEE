...
class iRMB(nn.Module):
    def __init__(
        ...
        self.se = (
            nn.Sequential(
                nn.AdaptiveAvgPool2d(1),
                nn.Conv2d(dim_out, int(dim_out * se_ratio), 1),
                nn.ReLU(inplace=True),
                nn.Conv2d(int(dim_out * se_ratio), dim_out, 1),
                nn.Sigmoid(),
            )
            if se_ratio > 0
            else nn.Identity()
...
class C3k2_iRMB(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, g=1, shortcut=True, se_ratio=0.25):
        """Initializes the C3k2 module, a faster CSP Bottleneck with 2 convolutions and optional C3k blocks."""
        super().__init__(c1, c2, n, shortcut, g)
        self.m = nn.ModuleList(iRMB(self.c, self.c, 2, shortcut, se_ratio=se_ratio) for _ in range(n))
