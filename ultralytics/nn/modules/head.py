...
from .elconv import ELConv, LDConv
...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        #     nn.Sequential(
        #         (LSConv(x, c2) if i == 0 else Conv(x, c2, 3)),  # P3 用 LSConv，其余层用 Conv
        #         Conv(c2, c2, 3),
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(ELConv(x, c2), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
...
class v10Detect(Detect):
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(LSConv(x, c2), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
