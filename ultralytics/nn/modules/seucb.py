...
class SEUCB(nn.Module):#EUCBS模块
    """
    Efficient Up-Convolution Block (改进版)
    - 上采样 + DWConv + Shuffle + PWC
    - 改进点：
      1. 上采样支持 bilinear，避免小目标锯齿
      2. DWConv + BN + Act 后增加 PWC + BN + Act，类似轻量 DSConv
      3. Channel Shuffle 改成 groups 可调（默认2）
      4. 残差连接 (若通道一致)
      5. 默认激活改 SiLU
    """
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 activation='silu', shuffle_groups=2, use_residual=True, up_mode="bilinear"):
        super(SEUCB, self).__init__()
