...
1.  **C3k2iMPF (Backbone)**: Integrates the _Inverted Residual Mobile Block (iRMB)_ with a _Multi-scale Pooling Fusion (MPF)_ mechanism. It strengthens small-object feature extraction while drastically reducing complexity.![iMPF](figs/iMPF.svg)

2.  **SEUCB (Neck)**: The _Small Efficient Up-Convolution Block_ optimizes the upsampling process, enabling efficient multi-scale feature representation and transmission.
    ![SEUCB](figs/SEUCB.svg)
3.  **ELConv (Convolution)**: An _Enhanced Lightweight Convolution_ module tailored for small objects, balancing precision with real-time processing requirements.
